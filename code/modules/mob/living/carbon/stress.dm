...
/mob/living/carbon/update_stress()
	...
	if(new_stress != oldstress)
		...
		if(diff_abs > 1)
			if(ascending)
				to_chat(src, span_smallred("Я чувствую накапливающийся стресс."))
				if(!rogue_sneaking || alpha >= 100)
					play_stress_indicator()
			else
				to_chat(src, span_smallgreen("Я чувствую облегчение на душе."))
				if(!rogue_sneaking || alpha >= 100)
	...
	if(old_threshold != new_threshold)
		switch(new_threshold)
			if(STRESS_THRESHOLD_NICE)
				to_chat(src, span_green("Я чувствую себя хорошо"))
			if(STRESS_THRESHOLD_NEUTRAL)
				if(ascending)
					to_chat(src, span_info("Я чувствую себя все хуже..."))
				else
					to_chat(src, span_info("Я больше не чувствую себя настолько напряженн[src.gender == FEMALE ? "ым" : "ой"]."))
			if(STRESS_THRESHOLD_STRESSED)
				if(ascending)
					to_chat(src, span_red("Я испытываю сильный стресс..."))
				else
					to_chat(src, span_notice("Мне стало немного легче, на какое-то время."))
			if(STRESS_THRESHOLD_STRESSED_BAD)
				if(ascending)
					to_chat(src, span_boldred("Я уже на пределе!"))
				else
					to_chat(src, span_boldnotice("Я больше не волнуюсь так сильно..."))
			if(STRESS_THRESHOLD_FREAKING_OUT)
				to_chat(src, span_boldred("Я СХОЖУ С УМА!!!"))
