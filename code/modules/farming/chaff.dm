...
/obj/item/natural/chaff/attack_right(mob/user)
	if(foodextracted && !user.get_active_held_item())
		to_chat(user, span_warning("Я начинаю трясти [src]..."))
		if(move_after(user,40, target = src)) //ROGTODO make this based on farming skill and speed
			user.visible_message(span_notice("[user] встряхивает [src]."), \
								span_notice("Я встряхиваю [src]."))
			testing("1")
				...
				if(isturf(loc))
					...
					for(var/obj/item/natural/chaff/R in loc)
						if(W.forked.len <= 19)
							...
							stuff++
					if(stuff)
						to_chat(user, span_notice("Я собираю стебли вилами."))
					else
						to_chat(user, span_warning("В моих вилах уже достаточно стеблей."))
					W.update_icon()
	...
	if(istype(I, /obj/item/rogueweapon/mace/woodclub))//reused some commented out code
		...
		for(var/obj/item/natural/chaff/C in get_turf(src))
			...
			if(prob(threshchance))
				C.thresh()
		user.visible_message(span_notice("[user] обмолачивает стебли!"), \
							span_notice("Я обмолачиваю стебли."))
