...
/obj/item/natural/worms/leech/examine(mob/user)
	...
	switch(blood_storage/blood_maximum)
		if(0.8 to INFINITY)
			. += span_bloody("<B>Она разбухла и налилась кровью.</B>")
		if(0.5 to 0.8)
			. += span_bloody("Она хорошо накормлена.")
		if(0.1 to 0.5)
			. += span_warning("Она жаждет крови.")
		if(-INFINITY to 0.1)
			. += span_dead("Она изголодалась.")
	if(!giving)
		. += span_warning("Она [pick("хлебает", "сосёт", "втягивает")].")
	else
		. += span_notice("Она [pick("изрыгает", "изрыгает", "источает")].")
	if(drainage)
		START_PROCESSING(SSobj, src)
