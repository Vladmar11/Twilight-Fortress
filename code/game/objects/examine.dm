...
/obj/item/examine(mob/user) //This might be spammy. Remove?
	...
	if(max_integrity)
		if(obj_integrity < max_integrity)
			...
			switch(meme)
				if(0 to 1)
					. += span_warning("Сломано.")
				if(1 to 10)
					. += span_warning("Вот-вот сломается!")
				if(10 to 30)
					. += span_warning("Видны тяжелые повреждения.")
				if(30 to 80)
					. += span_warning("Видны повреждения.")
				if(80 to 99)
					. += span_warning("Видны небольшие повреждения.")
...
//		. += span_notice("<a href='?src=[REF(src)];inspect=1'>Inspect</a>")
	...
	if(real_value > 0)
		if(HAS_TRAIT(user, TRAIT_SEEPRICES) || simpleton_price)
			. += span_info("Ценность: [real_value] маммон")
			...
			. += span_info("Ценность: [fumbled_value] маммон... <i>Мне так кажется</i>")
